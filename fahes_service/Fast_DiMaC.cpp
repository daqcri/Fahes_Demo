...
// ========================================================================
long F_DiMaC::compute_num_cooccur(const vector<string> & Vec, const long & idx){
    long num_coocur = 0;
    long index_min_subT;
    long min_subT;
...
// ========================================================================
long F_DiMaC::compute_num_occur(string v, long A){
    map<string, doubleVecStr>::iterator index_itr12 = DiMaC_Index_T[A].find(v);
    return index_itr12->second.size();
}

// ========================================================================
long F_DiMaC::compute_num_cooccur(const vector<string> & Vec, const long & idx,
                                    DiMaC_Index & DiMaC_Index_subT){
    long num_coocur = 0;
    long index_min_subT;
    long min_subT;
    bool first_time = true;
...
// ========================================================================
long F_DiMaC::compute_num_occur(string v, long A, DiMaC_Index & DiMaC_Index_subT){
    map<string, doubleVecStr>::iterator index_itr12 = DiMaC_Index_subT[A].find(v);
    return index_itr12->second.size();
}
// ========================================================================
bool F_DiMaC::prune_attribute(const long idx, vector<map<string, long> > & M){
    if (M[idx].size() < 3)
        return true;
...
// ========================================================================
double F_DiMaC::subtable_correlation(const Table & T, const string mc,
                                        const long idx, long & num_rows_in_subtable){
    double corr = 0;
    map<string, doubleVecStr>::iterator index_itr = DiMaC_Index_T[idx].find(mc);
    Table TP; 
    TP.header = T.header;
    ...
    DiMaC_Index DiMaC_Index_subT;
    Table_Index_DiMaC(TP, DiMaC_Index_subT);
    num_rows_in_subtable = index_itr->second.size();
    for (long i = 0; i < TP.number_of_rows; i++){
        corr +=  record_correlation(TP.data[i], T, TP, DiMaC_Index_subT, idx);
    }
    return corr;
}

// ========================================================================
// double F_DiMaC::Attribute_correlation(const Table & T, const Table & TP, DiMaC_Index & DiMaC_Index_subT,
//                                         const long idx){
...
//                 corr +=  record_correlation(TP[i], T, TP, DiMaC_Index_subT, idx);
//             // }
//         }
//     return corr;
// }
// ============== SELECT subtable when attribute equals a given value =========
// Table F_DiMaC::SELECT(const Table & T, const string & v, const long & A){
//     Table TT;
//     TT.table_name = T.table_name;
//     TT.header = T.header;
//     map<string, doubleVecStr>::iterator index_itr = DiMaC_Index_T[A].find(v);
...
// }
// ========================================================================
double F_DiMaC::record_correlation(const vector<string> & Vec, const Table & T, const Table & TP, 
    ...
    n_V_pt = compute_num_cooccur(Vec, idx, DiMaC_Index_subT);
    for (long i = 0; i < n; i ++)
        if (i != idx)   n_vi_pt[i] = compute_num_occur(Vec[i], i, DiMaC_Index_subT);
...
// ========================================================================
void F_DiMaC::Table_Index_DiMaC(const Table & T, DiMaC_Index & IDX){
    // DiMaC_Index IDX;
...
// ========================================================================
vector< sus_disguised > F_DiMaC::find_disguised_values(const Table & T,
    ...
    Table_Index_DiMaC(Temp_T, DiMaC_Index_T);
