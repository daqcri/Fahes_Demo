...
// ==============================================================
vector < map<string, long> > pattern_learner::find_all_patterns(vector<map<string, long> > & tabhist, 
										TableProfile & TP,
										vector<sus_disguised> & sus_dis_values){
	map<string, long> col_hist;
	vector<vector<string> > ptrns_vec;
	map<string, long> ptrns_hist;
	vector < map<string, long> > all_atts_ptrns;
	while (!all_atts_ptrns.empty())		all_atts_ptrns.pop_back();
	...
	for (long i = 0; i < (long)tabhist.size(); i++) {
		...
		if (ptrns_hist.size() > min_num_ptrns){
			L2_patterns(ptrns_vec, ptrns_hist);			
			agg_itr->second++;
		}
		if (ptrns_hist.size() > min_num_ptrns){
			L3_patterns(ptrns_vec, ptrns_hist);
			agg_itr->second++;
		}
		if (ptrns_hist.size() > min_num_ptrns){
			L4_patterns(ptrns_vec, ptrns_hist);
			agg_itr->second++;
		}
		if (ptrns_hist.size() > min_num_ptrns){
			L5_patterns(ptrns_vec, ptrns_hist);
			agg_itr->second++;
		}
...
void pattern_learner::L1_patterns(map<string, long> & colhist,
	...
	for (itr = colhist.begin(); itr != colhist.end(); itr++){
		...
		ptrn_hist_itr = ptrns_hist.find(ptrn);
		if (ptrn_hist_itr == ptrns_hist.end()) {
	...
	// cout << "number of unique patterns = " << ptrns_hist.size() << endl;
...
vector<string> pattern_learner::L2_patterns(vector<vector<string> > & pttrns,
	...
	for (long i = 0; i < (long) pttrns.size(); i++){
		...
		new_ptrn_hist_itr = new_ptrns_hist.find(new_ptrn_str);
		old_ptrn_hist_itr = old_ptrns_hist.find(old_ptrn_str);
		if (new_ptrn_hist_itr == new_ptrns_hist.end()) {
			new_ptrns_hist[new_ptrn_str] = old_ptrn_hist_itr->second;
			reduced_pttrns_vec.push_back(new_ptrn);
		} else {
			new_ptrn_hist_itr->second += old_ptrn_hist_itr->second;
		}
	}
	old_ptrns_hist = new_ptrns_hist;
...
// ==============================================================
void pattern_learner::determine_dominating_patterns(map<string, long> ptrns_hist, 
	...
	map<string, long>::iterator itr = ptrns_hist.begin();
	long total_dist_vals = 0;
	
	while(itr != ptrns_hist.end()){
