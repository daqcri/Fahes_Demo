	...
	for (long i = 0; i < (long)TP.header.size(); i++){
			...
			if (L_Nums > 2){
				...
				for (long j = 0; j < L_Str; j++){
					...
					if (!(string_itr2 == TP.profile[i].common_Strings.end())){
						add_sus_struct(TP.header[i], s, sc, string_itr->second, "SYN", sus_dis_values);
		...
		for (long k = 0; k < L_Str1; k++){
			...
			if ((s.length() >= 5) && (s.length() < 30)){
				...
				if((s.substr(0,4) == "some") || (s.substr(0,5) == "other")){
					add_sus_struct(TP.header[i], string_itr->first, sc, string_itr->second, "Rand", sus_dis_values);
						// if (!member_of(sus_dis, sus_dis_values))
						// 	sus_dis_values.push_back(sus_dis);
				}
				string_itr ++;
	...
	for (long i = 0; i < (long)TP.header.size(); i++){
		...
		if ((num_positives == 1) && (num_negatives > number_of_diff_ele)){
			...
			if (!(string_itr == TP.profile[i].common_Strings.end())){
				add_sus_struct(TP.header[i], str, sc, double_itr->second, "SYN", sus_dis_values);
				// if (!member_of(sus_dis, sus_dis_values))
				// 	sus_dis_values.push_back(sus_dis);
			}
		}
